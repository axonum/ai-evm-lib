...
library AILib {
    ...
    function onnxInference(bytes memory model, bytes memory input_data, uint256 output_size) internal view returns (bytes memory) {
        ...
        assembly {
            // call the precompiled contract opmlInference (0xa1)
            let success := staticcall(gas(), 0xa0, add(input, 0x20), len, output, retLen)
